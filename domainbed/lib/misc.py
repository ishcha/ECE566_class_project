...
class _SplitDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, key):
        sample = self.underlying_dataset[self.keys[key]]
        return sample
    def __len__(self):
...
def split_dataset_2(dataset, n, seed=0):
    assert(n <= len(dataset))
    # keys = list(range(len(dataset)))
    targets = dataset.tensors[1]
    zero = list(np.where(np.array(targets) == 0)[0])    
    np.random.RandomState(seed).shuffle(zero)
    one = list(np.where(np.array(targets) == 1)[0])
    keys_1 = zero[:n]
    keys_2 = zero[n:] + one
    # print(keys_1, [dataset[i][1] for i in (keys_1)])
    # wetoiu
    return _SplitDataset(dataset, keys_1), _SplitDataset(dataset, keys_2)
